...
public class ConsultarSegmentacion {
    ...
    public String detalle() {
        try{
            CampaniasSegmentacion cs = (CampaniasSegmentacion)getBindingTabla().getSelectedRowData();
            Util.setValuePageFlowScope("segmentonombre",cs.getNombre()+".xlsx");
            Util.setValuePageFlowScope("excelsegmento",Util.getRealPath(File.separator +"reportes" + File.separator + cs.getNombre().trim() + ".xls"));
            ((ExpertoSegmentacionLocal)CachingServiceLocator.getInstance().lookup("ExpertoSegmentacion")).getSegmentacionBase(cs.getId(), (String)Util.getValuePageFlowScope("excelsegmento"));            
    ...
    public void tablaSelectionListener(SelectionEvent selectionEvent) {
        Util.setValuePageFlowScope("segmento", getBindingTabla().getSelectedRowData());
        if(Util.getValuePageFlowScope("segmento")!=null){
            List<String> datos = new ArrayList<String>();
            Map map = Util.jsonParse(((CampaniasSegmentacion)Util.getValuePageFlowScope("segmento")).getFiltros());
    ...
    public void downloadAction(FacesContext facesContext, OutputStream outputStream) {
        try {
            System.out.println((String)Util.getValuePageFlowScope("excelsegmento"));
