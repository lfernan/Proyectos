...
public class GestionarCampanias {
    ...
    public GestionarCampanias(){
        try {
            ...
            //setExpertoCampanias((ExpertoCamapaniasLocal)new InitialContext().lookup("java:comp/env/ExpertoCamapanias"));
    ...
    public String preFiltro(){        
        try {
           getExpertoCampanias().cargarBaseClientes(null, (Campanias)Util.getValuePageFlowScope("campania"), (List<BaseFiltradaDTO>)Util.getValuePageFlowScope("prefiltro"));            
           cancelar();
           actualizarDatos();
       } catch (Exception e) {            
            e.printStackTrace();
    ...
    private void actualizarDatos() {
        try {            
            getBindingTabla().setValue(
                                getExpertoCampanias().find(
                                Util.getValueSessionScope("descripcion")==null?null:(String)Util.getValueSessionScope("descripcion"), 
                                Util.getValueSessionScope("tipo")==null?null:((TipoCampanias)Util.getValueSessionScope("tipo")).getTipocampId(), 
                                Util.getValueSessionScope("fdesde")==null?null:(Date)Util.getValueSessionScope("fdesde"), 
                                (Date)Util.getValueSessionScope("fhasta")==null?null:(Date)Util.getValueSessionScope("fhasta"), 
                                null,true));
    ...
    public void tablaFiltroSelectionListener(SelectionEvent selectionEvent) {
        Util.setValuePageFlowScope("archivo", ((BaseFiltradaDTO)getBindingTablaFiltro().getSelectedRowData()).getArchivo());
    }
    ...
    public List<SelectItem> getItemProductos() {
        List<SelectItem> l = new ArrayList<SelectItem>();
        try{
            for(ConsultaDTO o: getExpertoCampanias().getProductos()){
    ...
    public String detalle() {
        try {
            Util.setValuePageFlowScope("archivo",ReportesBean.generarReporteBaseClientes((Campanias)Util.getValueSessionScope("campania")));
    ...
    private Long procesar(Campanias campania) {
        try {
            ...
            Util.setValueSessionScope("path",Util.getRealPath(File.separator +"reportes" + File.separator));
            if(getBindingProducto().getValue()!=null){
                List<BaseFiltradaDTO> pf = getExpertoCampanias().filtrarBaseClientes(documentos,campania,
                                                                                     (ConsultaDTO)getBindingProducto().getValue(),
                                                                                     (String)Util.getValueSessionScope("path"));
    ...
    public String segmentar() {
        if(Util.getValueSessionScope("campania")!=null&&Util.fechaToLong(((Campanias)Util.getValueSessionScope("campania")).getCampFechaVigenciaHasta())<Util.fechaToLong(new Date())){
    ...
    public void tipoChangeListener(ValueChangeEvent valueChangeEvent) {
        if(getBindingTipo()!=null && getBindingTipo().getValue() != null && ((TipoCampanias)getBindingTipo().getValue()).getTipocampId() == 1){
            ...
            getBindingProducto().setDisabled(true);
            getBindingProducto().setRequired(false);
        }else if(getBindingTipo()!=null && getBindingTipo().getValue() != null && ((TipoCampanias)getBindingTipo().getValue()).getTipocampId() == 2){
    ...
    public Date getMaxFechaHasta(){
        ...
        if(getBindingTipo().getValue()!= null && ((TipoCampanias)getBindingTipo().getValue()).getTipocampId() == 2){                            
            if(getBindingFechaDesde().getValue()!= null){
                c2.clear();
                c.setTime((Date)getBindingFechaDesde().getValue());                                
    ...
    public boolean getRenderSegmentar(){
        if(session != null && /*session.accionHabilitada("11201", "1")*/true && getBindingSegmentar() != null){
            return true;
        }else{
            return false;
        }
