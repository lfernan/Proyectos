...
@XmlType(name = "MensajeDTO", propOrder = {
         ...
         "referencia", "usuarioNombre", "importancia", "clasificacion", "producto", "tipoDeOrigen", "horaDeCarga",
         "usuarioLegajo"
    })
public class MensajeDTO implements Serializable {
    ...
    public String getFechaCompFormateada() {
        if(fechaDeCompromiso!=null){
            Calendar cal = Calendar.getInstance();
            cal.clear();
            cal.set(Calendar.YEAR, fechaDeCompromiso.getYear());
            cal.set(Calendar.MONTH, fechaDeCompromiso.getMonth() - 1);
            cal.set(Calendar.DATE, fechaDeCompromiso.getDay());
            Date date = cal.getTime();
            SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
            return sdf.format(date);
        }else{
